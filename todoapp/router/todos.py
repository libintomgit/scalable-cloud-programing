...
async def read_all_by_user(request: Request, db: Session = Depends(get_db)):
    ...
    return templetes.TemplateResponse("home.html", {"request": request, "todos": todos})

@router.get("/add-todo", response_class=HTMLResponse)
async def add_new_todo(request: Request):
    return templetes.TemplateResponse("add-todo.html", {"request": request})
...
async def create_todo(request: Request, title: str = Form(...), description: str = Form(...), priority: int = Form(...), db: Session = Depends(get_db)):
    ...
    return RedirectResponse(url="/api/todo", status_code=status.HTTP_302_FOUND)
...
async def edit_todo(request: Request, todo_id: int, db: Session = Depends(get_db)):
    ...
    return templetes.TemplateResponse("edit-todo.html", {"request": request, "todo": todo})


@router.post("/edit-todo/{todo_id}", response_class=HTMLResponse)
async def edit_toto_commit(requst: Request, todo_id: int, title: str = Form(...), description: str = Form(...), priority: int = Form(...), db: Session = Depends(get_db)):
...
@router.get("/completed/{todo_id}", response_class=HTTPException)
async def comlete_todo(request: Request, todo_id: int, db: Session = Depends(get_db)):
